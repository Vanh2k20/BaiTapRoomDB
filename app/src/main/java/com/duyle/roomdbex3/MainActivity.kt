    ...
    Column (Modifier.fillMaxWidth()){
        ...
        Button(onClick = {
            if (hoTen.trim().equals("") || mssv.trim().equals("") || diemTB.isEmpty() || daRaTruong.isEmpty()){
                Toast.makeText(context,"Chưa nhập dữ liệu thông tin!", Toast.LENGTH_SHORT).show()
            }else if (!isFloat(value = diemTB)){
                Toast.makeText(context,"Điểm trung bình nhập chưa đúng!", Toast.LENGTH_SHORT).show()
            }else if (10f < diemTB.toFloat() || diemTB.toFloat() < 0f){
                Toast.makeText(context,"Điểm trung bình phải từ '0 - 10'", Toast.LENGTH_SHORT).show()
            }else if (!isBoolean(daRaTruong)){
                Toast.makeText(context,"Đã ra trường nhập 1 là 'Đã ra', 2 là 'Chưa ra'!", Toast.LENGTH_SHORT).show()
            }else {
                ...
                listStudents = db.studentDAO().getAll()
                Toast.makeText(context,"Thêm sinh viên mới thành công", Toast.LENGTH_SHORT).show()
            }
        }) {
        ...
        LazyColumn {
            ...
            items(listStudents) {
                Row (
                    ...
                    Text(modifier = Modifier.weight(1f), text =if(it.daratruong!!) "Đã ra" else "Chưa ra")
                }
                Divider()
            }
        }
